...
$sql="INSERT INTO `degree`(`DName`, `Fees`, `Created_By`, `Created_At`) VALUES 
							('{$data->name}',{$data->fee},1,NOW())";
//$results=mysqli_query($conn,$mysql);
	if (mysqli_query($conn,$sql)){
		
	header('Location:degreelist.php');
}
else print_r($sql);
}
...
<!------ Include the above in your HEAD tag ---------->
	...
	<div class="container">
			<div class="main">
				...
				<input type="text" class="form-control" name="name" id="name"  placeholder="Enter Degree Name"/>
							</div>
						</div>
						...
						<div class="form-group">
							<label for="email">Fee</label>
								<div class="input-group">
									...
									<input type="number"  min="100000" max="5000000" class="form-control" name="fee" placeholder="Enter Fee"/>
							</div>
						</div>
